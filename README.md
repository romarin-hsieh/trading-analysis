...
| Then what's the asset here? | **The method and the data moat.** 39 adversarially audited test reports, a registry where negative results are first-class, seven documented self-corrections, and a $0 data stack with two collect-forward pipelines recording what money can't buy retroactively. |
...
| 有選股訊號嗎？ | **有一個，而且愈測愈誠實。**毛利/資產品質因子——頭條 ICIR +0.30，但遮罩到「當天真的在指數裡」的名字後只保留 59%(ICIR +0.13，TR-27)，且 2025–26 的 IC 轉負(列入觀察)；聯合 Fama-MacBeth 面板把觀察磨得更利——2015–2020 獨立被定價(t=+2.67)，2021 之後歸零(TR-34)。動能、價值、PEAD、內部人、機器學習預測、四大經典異象全數陣亡；產業動量能複製但整條是動量因子曝險(TR-32)。而且兩個倖存者疊不起來：把 GP sleeve 加進主力帳簿反而扣掉 alpha(TR-33——誠實鏈的終點是 +0.04%/yr 的可交易價差)。 |
| 擇時有用嗎？ | **沒有——七次獨立確認。**一個恆定部位或隨機進場對照，就能追平或打敗我們測過的每一個擇時機制(Markov、IBS、吸收比率兩個座位、KMZ 複雜度、Campbell-Thompson 符號約束、Larry Williams 外包線)。 |
| 那這個專案的資產是什麼？ | **方法本身，加上資料護城河。**39 份經對抗式稽核的測試報告、一份把負面結果當一等公民的註冊表、七次白紙黑字的自我修正，以及 $0 的資料堆疊與兩條「現在不錄、以後買不到」的前向收集管線。 |
