...
| **Larry Williams 外包線反轉**(影片一 #8) | Larry Williams《Long-Term Secrets》ch.7 | **→ TR-30 已執行:FAILED(NO-EDGE);使用者稽核抓到引擎四處不忠實→ [TR-30b](tests/TR-30b-outside-bar-faithful.md) 忠實重測,同類判定(勝率宣稱被定位重現:緩衝 0.2→3.0×ATR 勝率 35%→81%;進場零增值)** |
...
**Tito 的「實驗室思維」= 我們 fabric 的哲學**:假設→實驗→記錄→調變數→從失敗學,也是一個錦標賽
冠軍的流程反過來驗證了我們的方法論(非新機制,但是很好的敘事佐證)。

**元教訓補充(承 §5)**:創作者影片也適用 **T1 機器忠實度**——動工前先從逐字稿抽出完整規則表
(訊號過濾/進場/止損/止盈,逐條映射到引擎元件);含未公布參數的宣稱用 CAL-locate 在預先宣告的
網格中定位;判進場價值時安慰劑必須共用出場引擎。TR-30 跳過此步,被**使用者稽核**抓到(詳 TR-30b)。
創作者影片管線第二次產出 TR(前例 TR-21 吸收比率),兩次都 FAILED——影片是線索非證據的命題再獲
確認。價值排序:合輯類影片的真正用途是「快速盤點哪些流行機制已被我們測過」(免得重複勞動),
而非期待新 alpha。
