...
## 附錄 TR-04b:Student-t ν 定量(2026-07-09,經對抗稽核修正)

`scripts/tests/tr04b_student_t.py`:**無條件 ν = 2.55–3.04**(QQQ 1999-/QQQ 2015-/SPY 2015-;
低於引文的 [3,5],因無條件分布混入波動叢聚)、**條件 ν(EWMA 標準化)= 4.6–6.6** 才是與經典
引文可比的量。99% 層 t-VaR 對經驗 1% 分位校準良好、常態低估 ~20–25%;99.9% 的 +107~150% 是
**參數外插**(QQQ-1999 樣本內 t-VaR99.9 被突破 0 次 vs 期望 6.9;經驗 0.1% 差距 ~+59%),方向
真(常態在 99.9% 違規 ~7× 名目)、幅度模型相依。**歸因校正**:靜態肥尾只解釋 Kupiec 失敗的一部
分(隱含違規率 1.6–1.8%),其餘至 2.9% 來自波動叢聚+滾動視窗滯後——與本文主判定(常態 VaR 全滅、
用歷史/CF)一致並補上機制拆解。
