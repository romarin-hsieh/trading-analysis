...
|---|---|---|---|---|---|---|
| +5d | 164 | +0.5% / −0.6% | 47% | +1.7% | −1.1% | 第 19 百分位 |
| +21d | 164 | +5.1% / −1.1% | 45% | +5.4% | −0.3% | 第 56 百分位 |
| +63d | 154 | +10.2% / +0.8% | 51% | **+20.4%** | **−10.2%** | **第 2.4 百分位** |

**四個誠實結論**:
1. **正超額全來自「宇宙」,不是「擇時」**:隨機日期買進他的股票池,63 天贏 QQQ +20.4%——他的真實 call 時點反而只有 +10.2%,**顯著低於隨機**(部分因誤判產生的「遲到進場」,見 3)。跟單的價值=**他的選股宇宙**(光子/neocloud/記憶體鏈),不是他喊單的時機。
2. **顯著性經不起校正**:plain t=2.34 好看,但事件擠在同段牛市(21% 在 2026-01),**月度聚類校正後 t≈1.0-1.2、bootstrap P=0.08-0.16 → 與零無法區分**;且**前 5 個事件(AXTI +297%/SNDK +276%/AEHR +216%…)貢獻了 ~70% 的全部超額**,拿掉後均值 ≈−1~+3%、**中位數為負、勝率 43-50%——超過一半的 call 輸給 QQQ**,彩券型分布。
3. **關鍵字分類噪音大到不能歸因於「他」**:人工抽查 ~60% 的「轉多」標籤是誤判(「Never long though」被標多、清單文誤植、勝利回顧文比真 call 晚 2 個月→機械性壓低他的前向報酬)。已修兩輪真 bug:(a) `tp` 子字串匹配到 `http`,16.8% 含連結推文永遠不能標多(→ 詞界 regex + 先剝 URL);(b) 否定/非方向片語誤標多(「never long」「no positions」「long-term」「as long as」→ 加否定守衛;367 個 first_long 事件中 344 不變、6 檔日期後移、17 檔消失,結論不變且更乾淨)。殘餘噪音(弱關鍵字佔 54%、多 ticker 推文整條繼承同方向)深層修復需 LLM 逐則分類(Haiku batch ~$1/月,已列升級路徑)。
