...
| gross_profitability 品質因子 | ICIR +0.30 跨期同號、regime-universal(bear 最強);TR-24B 再確認 subsumes ROE。**TR-26 深度確認(ROBUST-PLATEAU):持有期單調(ICIR 21d 0.10→252d 0.41=慢因子)、98% 半宇宙為正、毛多空僅 ~1%/yr=訊號非獨立策略**。**⚠️ WATCH:IC 2025-26 轉負——TR-26 量化:滾動 −0.050 vs 歷史最深 −0.109(2022,一年內復原)**;2026 全年仍負則啟動 F10 複測 | docs/10、TR-24、**TR-26** | `scripts/fundamental_factors.py`、`tr26_gp_depth_grid.py` |
...
### 🧪 標準化測試報告 TR-01~26 + 03b/04b/17b/21b(詳見 docs/tests/;**docs/20 論文佇列全清**;TR-01~08 由稽核員重跑全部腳本、數字一對一吻合)
| TR | 機制 | 判定 | 摘要 |
|---|---|---|---|
