...
### 🧪 標準化測試報告 TR-01~52 + 03b/04b/17b/21b/30b/39b(詳見 docs/tests/;**docs/20 論文佇列全清**;TR-01~08 由稽核員重跑全部腳本、數字一對一吻合)
| TR | 機制 | 判定 | 摘要 |
|---|---|---|---|
...
| TR-40 | (⚠️ TR-44 總報酬修正:logdv 淨 t 由 2.33 退為 1.86=MARGINAL) 台股候選成本關卡(docs/27 b2;純多頭頂十分位、月再平衡、手續費 0.1425%×2+證交稅 0.30%+價差) | **SURVIVES: logdv | MARGINAL: mom122/avol** | **成本後排序徹底翻轉:FM 最強的 avol(t=3.39)被 6.8× 年換手吃掉 7.07% 成本→淨 +1.93%(t=0.59);最弱的 logdv(t=−2.77)因 0.9× 換手只吃 0.97%→淨 +6.75%(t=2.33)存活**;換手率是唯一樞紐(TR-29 持有期教訓在新棲地重演);**CAL-b 兩次否決 CS 成本模型**——即使真實高低價,CS 仍把薄量股讀成價差較窄(47 vs 66bps,無成交日壓平區間)=CS 在台股薄量端失效→改機械式 tick 模型(2 跳基準+1/2/4 敏感度);**POST-RUN 容量診斷:logdv 損益兩平價差 ≈31 跳(極穩健)但全組合容量僅 ≈NT$1,500 萬**(每檔日成交 NT$1.5M)=低流動性溢酬的定義(付錢給你因為你出不來);avol 容量 NT$29 億但淨額≈0=同一條供需曲線兩端 | 
| TR-41 | 台股桶經濟性:溢酬住在哪(docs/27 b3;十分位階梯+兩實作對決+容量前緣) | **CONCENTRATED** | CAL 過(D1 +6.65% vs TR-40 +6.75%);**溢酬只在 D1:D2 已歸零(−0.35%)、D3-D9 全負(最深 D8 −6.25%,t=−3.90)**;**「避開最熱門」(D1-D9)是顯著虧損 −0.92%/yr(t=−2.34)——排除好桶=重壓壞桶,直覺被證偽**;C3 黃金診斷=**啞鈴**(兩端 +6.81%/+1.94% 勝中段 −2.34%),與 TR-33 美股 GP 的「兩端輸中段」方向相反;**跳一個月檢查通過(+6.61%,t=2.31)=不是停滯價/價差彈跳假象**;C4 容量前緣:NT$5,000 萬 +5.29%、NT$1.5 億 +3.19%、NT$5 億 +1.48%(填滿率 19%)=**無高容量替代方案**;三特徵(集中尾端、隨規模單調衰減、通過停滯價檢查)全符 Amihud-Mendelson 流動性補償而非異象 | 
| TR-42 | L2 股債相關 regime 煞車(風險引擎 v2 第一挑戰者;docs/25 C1) | **EIGHTH TIMING FAILURE** | CAL 過(類比座位重現 TR-35 MDD −14.6%);訊號=36 月滾動股債相關>0(佔 48% 月份,落後一期);**煞車A Calmar 0.57 輸給同平均曝險恆定部位 0.68(恆定在兩軸都贏:MDD −12.6% vs −14.4%、CAGR 8.5% vs 8.3%),且只在隨機煞車安慰劑第 20 百分位=比隨便踩還差**;煞車B(債換金)Calmar 0.41/MDD −25.5% 更糟(金波動;TR-35「金扛通膨」是單腿事實,非組合層可行);**C3 誠實記錄:煞車在設計窗確實有效(停滯性通膨比率 1.04→0.74、1994 1.16→0.82)但 48% 破裂月的機會成本吃光一切**=七次失敗的同一機制;**鐵律 7→8 並擴及「風險輸入型擇時」**(不只別預測報酬,是別依任何 regime 訊號調曝險);主力不動,風險引擎優先序改 L3>L1>L4 | 
| TR-43 | L3 EVT 尾部模組(風險引擎 v2 第二挑戰者;條件式 GPD vs 現役 Student-t vs 歷史模擬) | **REJECTED** | CAL 過(重現 TR-04b:常態 VaR 99% 被拒 p<1e-4、Student-t 未被拒 p=0.267);**實際座位:EVT 在 99%/99.5% 都被 Kupiec 拒絕(破線 1.82%/0.84%,p=0.000/0.036),現役都沒有(1.24%/0.44%,p=0.267/0.697)——EVT 低估尾部**;EVT 唯一優勢是 CVaR-RMSE(47 vs 90bps)=形狀較準但頻率錯了,無用;類比座位三模型無法分辨(皆過度保守);**C3 誠實條款:三模型的 L 刻度完全相同(0.11-0.12)=就算贏統計也改不了決策**(F0 預寫的 BETTER-BUT-INERT);診斷:EWMA 去波動化假設條件常態骨架,但本帳簿的極端日是**多腿相關跳升**而非單腿波動放大→GPD 從被壓平的殘差外推自然不夠寬(與 TR-05「尾部來源是相關結構非邊際波動」同源);**風險引擎 v2 目前 0/2**(L2 敗擇時鐵律、L3 敗估計檢定),框架如設計運作 | 
| TR-52 | VIX 之問資料收官(docs/28 t1;鐵律例外條款「選擇權資訊層」指數端的行使;VRP 預測 BTZ 2009+VIX/VIX3M 期限結構閘門,兩訊號一家族) | **IRON-LAW-9** | CAL:VIX 錨過、VRP 存在性過(mean>0、正月 86%)、**c v1 [65,92] 帶精準 FAIL→診斷=「~80% contango」屬期貨曲線,現貨指數比只在急性壓力倒掛→v2 重錨(92.3% ∈[65,97]+壓力月倒掛 4/5)**;s1 VRP:**IS t=+0.30、OOS R² −6.5%——BTZ 的預測力在延長窗連樣本內都消失**(發表後衰退第二例),組合層 Sharpe 0.61 vs B&H 0.73/波動目標 0.59、安慰劑 26 百分位 FAILED;s2 期限結構:Sharpe 0.89 vs B&H 0.92、**circular-shift 安慰劑 62 百分位=與隨機平移無法區分** FAILED;**鐵律 8→9,例外條款收窄至自建個股鏈(擇股用途;擇時通道全數關閉)** | 
| TR-51 | GP 去倖存終判(docs/27 B1;Tiingo 374 檔死名字價格+EDGAR/Form-4 CIK 橋 436 檔財報,F0 於補丁面板前預先登記 `7c1017a`,試驗 +0) | **WEAKENED——誠實鏈終點 +0.13 → +0.056** | CAL:未補集忠實(ICIR +0.165 ∈帶)、補丁咬合 48 檔、**c v1 空手 FAIL→v2 抓到兩個實作錯(SIVBQ→SIVB 代號、查找面板被基本面過濾)後 SBNY $0.40/SIVB $0.01 崩潰尾巴實在**;C1:補後 63d 成員 ICIR **+0.165→+0.056(Δ−0.109)=剩餘訊號 2/3 是倖存者眩光**,距 ARTIFACT 線 0.006;**C4 揭機制:兩個預先陳述偏誤方向都錯——缺席者=高 GP 衰退者(退場前 GP 分位 +0.57、相對報酬 −1.5%;標普剔除跌勢股而 GP 分母是帳面資產)**;C2 新 WATCH 基準:分期 +77.6(t=1.88)/−28.5(t=−0.90);C3 驗屍 MIXED(狀態差 t=−1.32、趨勢 t=−0.47,regime 與 crowding 皆不成立);**補後宇宙(1,180 檔含死亡尾巴)成為美股面板新標準宇宙** | 
| TR-50 | Hou-Moskowitz price delay(論文佇列 #4;W-WED 週報酬、六月年估 52 週、D1=1−R²受限/R²全模) | **INVERTED-BUT-ABSORBED** | CAL 過(中位 D1 0.155、市值前十分位 0.120<面板中位=規模梯度在 S&P500 內部成立、五分位單調 0.177→0.125);C1 單變量 **−89.5bps t=−3.20 符號與 HM 相反**(高 delay 大型股跑輸)但 C2 加六控制後 −31.5(t=−1.39)被 beta(+143 t=2.13)/規模吸收=**IVOL 家族方向的重新包裝非獨立訊號**;HM 溢酬本身不存在(與其原文規模表一致);依 F0 反向不升級 | 
| TR-49 | Daniel-Moskowitz 動量崩盤(論文佇列 #3;月頻 1927++日頻 1990+) | **REPLICATES + C4 判讀** | CAL-a v1 0.99 FAIL→**診斷=KF 日/月動量因子為兩個不同官方建構**(權重逐日漂移 vs 月形成,最大落差 2009-04)→v2 0.95+落差月須在崩盤年代 PASS;C1 最慘 10 月 **9/10 在熊市態**(1932-08 −53%、2009-04 −34%、2023-01 −16%);C2 熊市+高波動月均值 **−2.30%/mo(t=−2.41)** vs 無條件 +0.62%;C3 BSC 波動目標 Sharpe **0.42→0.96**/MDD −63%→−32%;**C4:DM-lite 崩盤預測權重未勝單純波動目標(1.02/−34% vs 0.98/−32%,兩軸未全勝)=「修復=波動管理,非崩盤預測」——鐵律形狀在因子座位的外部驗證(不計數)**;無可交易 sleeve | 
| TR-48 | LSV 1994 價值反向(論文佇列 #2;E/P、C/P、B/M、3 年銷售成長反向+雙向格) | **NO-SIGNAL** | CAL 過(中位 ep 4.7%/cp 7.6%/gs3 8%;corr(ep,bm)=+0.41、corr(gs3,bm)<0);C1 四路單變量全零(|t|≤0.68,覆蓋 383-392/mo);C2 LSV 簽名雙向格(低成長高C/P−高成長低C/P)+28bps t=0.90=組合不比零件強;C3 聯合裡 cp t=2.03 判**共線性分割**(單變量 0.68)依 F0 不升級;**價值通道在本座位升級為全家族陳述:B/M、E/P、C/P、成長反向、LSV 組合全零** | 
| TR-47 | De Bondt-Thaler 1985 長期反轉(論文佇列 #1;KF 慣例 t-60..t-13,價格庫深至 1990) | **NO-SIGNAL** | CAL 過(corr(ltrev,bm)=−0.35 長期輸家=價值股;KF 小型股腿 1931-82 **+48bps/mo t=2.68** 機器在效應存活處讀得出);C1 本座位 **−1.2bps t=−0.02**(覆蓋 455/mo)完美零;C2 聯合 −0.13;**C3 KF 年代弧=教科書 McLean-Pontiff 衰退:DBT 樣本期 +48/+42(小/大,皆顯著)→發表後 +11/+15(死)→2015+ −1/−30**——座位零不是量測失敗,是遲到 40 年 | 
| TR-46 | 美國內部人 Form 4(docs/28 p2;SEC 官方 81 季結構化資料集,CMP 機會型/例行拆解,F0 於訊號組裝前預先登記 `a5cca76`) | **NO-SIGNAL(有充分檢定力的零)** | CAL:**a v1 金額比 FAIL→診斷=Form 4 巨獸垃圾列支配年度加總(2017 比率 1457×、多年 ≈0),v2 改計數比 3.1 每年>1 PASS**(教訓:Form 4 金額聚合不可未 winsorize 使用);join 100%;**2020-03 危機錨命中(全市場 NPR +0.305 ≥ P90)=管線活的**;C1:npr6 t=+0.49、opp6 t=+0.33、rou6 t=+0.22(覆蓋 415/219/141 檔/月)——**CMP 型態不存在,opp≈rou=通道無內容非訊號弱**;C2 聯合 t=−2.25 判薄樣本共線性不可解讀(斜率量級爆炸 5-50×);C4 分期全平;O/R/U=14/18/68%(大型股內部人多無三年連續史);**美大型股資訊通道版圖:價格、基本面、內部人全關;collector+分類機器轉用於 p3 小型股腿** | 
| TR-45 | 台股籌碼線(docs/27 b7;法人買賣超+融資融券 1,220 檔,F0 於資料落地前預先登記 `bc3c1fb`) | **MECHANISM-CONFIRMED / MARGINAL** | CAL 全過(bottom-up 外資淨買 vs 官方聚合 corr **+0.996**、融資恆等式 100%、八特徵覆蓋 808 檔/月);C1:**fnet60 外資流延續 +78.4bps t=3.34**(唯一候選)、itnet60 +1.48、dmgn60 +1.85 且**符號與擁擠反轉先驗相反**(散戶槓桿也延續);C2 歸因:**mom/avol 幾乎不衰減(−7%/+10%)=延續市場不是外資流扛的**,fnet60 為獨立第四通道(聯合 t=2.91),max5 被籌碼完全吸收(+105%,樂透特徵=流向雜訊),logdv 衰減 42%(結構性補償再加固);C3:**as-run 絕對基準誤判 SIGNAL,對照 TR-40 原機器改超額基準後翻 FAILS-COSTS**(超額毛 +0.74%/yr 不過 ≥+40bps/mo 毛閘門、超額淨 −4.12% t=−2.51;駝峰+瀰散階梯,錢在放空側而台股借券受限);C4:冷角落籌碼全在中位=流動性溢酬非流向現象;**教訓入慣例:long-only 桶成本判定一律超額基準** | 
| TR-44 | 台股判定 × 還原股價(docs/27 b6;官方除權息前後價建還原因子,總報酬基準重跑全套) | **VERDICT-STABLE(機制)/ 但候選全退為 MARGINAL** | CAL 過(中位股利拖累 +3.23%/yr);**CAL-a 先抓到還原符號反置**(raw/factor 讓舊價膨脹→2330 還原 CAGR 反低於未還原,不可能;改 raw×factor);**C1 診斷:殖利率隨流動性單調上升(D10 4.01% > D1 3.13%)=未還原略高估 D1 相對優勢但僅 0.9pp**;C2 三候選符號與 |t|≥2 全穩(mom +86.7 t=2.53、avol +99.7 t=3.75、logdv −79 t=−2.15,logdv 減弱 25% 與診斷一致);C3 階梯仍懸崖+啞鈴(TR-41 複製);**但成本關卡後 logdv 淨 +5.32% t 降到 1.86<2=退為 MARGINAL**;C4 bp 不升反降(+40.8 t=1.32,高殖利率集中在流動大型股非價值股);**台股最終誠實狀態:機制為真但總報酬+全額成本下無一乾淨過關** | 
