...
def main():
    ...
    print("=" * 96)
    print(f"FACTOR DETERMINATION — us_study 51 names, forward {HORIZON}d return, 2015-2024")
    print("  (raw t inflated by overlapping windows; judge on IC sign-stability + monotonicity + incremental)")
    print("=" * 96)
    hdr = f"{'factor':16s} {'IC':>7s} {'ICIR':>6s} {'hit%':>5s} {'IC16-19':>8s} {'IC20-24':>8s} {'mono':>5s} {'incrIC':>7s} {'verdict'}"
    print(hdr)
    rows = []
    for name, fw in facs.items():
        ic = cross_sectional_ic(fw, fwd)
        summ = ic_summary(ic)
        ic1 = ic[ic.index < SPLIT]
        ic2 = ic[ic.index >= SPLIT]
        m1, m2 = float(ic1.mean()), float(ic2.mean())
        ...
        # robust pass: same-sign IC in BOTH sub-periods, monotone quantiles, |IC|>0.01
        ...
        rows.append((name, summ, m1, m2, mono, incr, verdict))
        print(f"{name:16s} {summ['mean_ic']:+7.3f} {summ['icir']:+6.2f} {summ['hit_rate']*100:4.0f}% "
              f"{m1:+8.3f} {m2:+8.3f} {mono!s:>5s} {incr:+7.3f} {verdict}")
    print("-" * 96)
    n = len(facs)
    print(f"multiple-testing: tested {n} factors -> deflate. With ~{n} trials a |IC|~0.02 'winner' can be luck;")
    print("  trust only factors PASSing on BOTH sub-periods + monotone + incremental-IC same sign as raw IC.")
    keep = [r[0] for r in rows if r[6] == "PASS"]
    print(f"KEEP (survives the gate): {keep if keep else 'NONE clears the full bar'}")
    print("=" * 96)
